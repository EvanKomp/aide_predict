...
def aggregate_and_null_per_fold(
    ...
    Returns a DataFrame with columns:
        ...
        y_pred, epi_std, ale_std, tot_std, n_refs,
...
def compute_beta_sweep_for_mode(mode_df: pd.DataFrame, beta_grid: List[float]) -> dict:
    ...
    For each β in ``beta_grid`` the rows are scored ``y_pred + β·tot_std`` and the
    six per-substrate-averaged ranking metrics are computed (with per-substrate
    breakdowns). The null prediction is uncertainty-free and therefore
    β-invariant: computed exactly once.
    ...
    for beta in betas:
        rm = _ranking_metrics_at_score(y_true, y_pred + beta * tot_std, subs)
...
def _per_substrate_recall_runs(
    ...
    acq = _acq_scores(y_pred, tot_std, best_beta)
...
def plot_per_position_recovery(
    ...
    acq_scores = _acq_scores(y_pred, tot_std, best_beta)
    bnn_label = f"BNN (β={best_beta:g})"
    label_map = {"BNN": bnn_label, "Null": "Null"}
