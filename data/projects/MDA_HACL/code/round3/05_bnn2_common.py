...
def aggregate_pairwise_predictions(
    ...
    for (mut, sub), grp in grouped:
        ...
        row_dict = {
            ...
            # Within-reference epistemic only (EXCLUDES the between-reference
            # disagreement var_means). Used to build the UCB acquisition sigma so
            # that β means the same thing for single- and multi-reference modes.
            # Additive column; reported _epi_std/_tot_std are unchanged.
            "_epi_within_std": np.sqrt(max(agg_epi_var_within, 0)),
            "n_refs": n_refs,
        }
